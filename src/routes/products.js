...
router.get('/', async (_req, res, next) => {
    ...
    try {
        ...
        if (productsAll) {
            res.render('pages/index.ejs', { productsAll })
        } else {
            res.status(404).json({
...
router.post('/', async (req, res, next) => {
    ...
    try {
        const { nameProduct, priceProduct, thumbnailProduct } = req.body
        const newProduct ={title:nameProduct,price:priceProduct,thumbnail:thumbnailProduct}
        const productSave = await products.save(newProduct)
        if (productSave) {
